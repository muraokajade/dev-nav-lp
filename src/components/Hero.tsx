...
const Stat: React.FC<{ k: string; v: string; sub?: string }> = ({ k, v, sub }) => (
...
export const Hero: React.FC = () => {
  ...
  const reactPos  = React.useMemo<[number, number, number]>(
    () => (isMobile ? [-1.5,  0.55, 0] : [-1.6,  0.35, 0]), [isMobile]
  );
  const tsPos     = React.useMemo<[number, number, number]>(
    () => (isMobile ? [-0.3, -1.00, 0] : [-0.3, -1.00, 0]), [isMobile]
  );
  const springPos = React.useMemo<[number, number, number]>(
    () => (isMobile ? [ 1.5,  0.40, 0] : [ 1.2,  0.25, 0]), [isMobile]
  );
  const javaPos   = React.useMemo<[number, number, number]>(
    () => (isMobile ? [ 2.6, -0.90, 0] : [ 2.1, -1.00, 0]), [isMobile]
  );

  // スケール
  const reactScale  = isMobile ? 2.00 : 2.30;
  const tsScale     = isMobile ? 0.85 : 0.7;
  const springScale = isMobile ? 2.00 : 2.30;
  const javaScale   = isMobile ? 1.55 : 1.20;
      ...
      <div className="grid md:grid-cols-2 gap-10 items-center">
        ...
        <div>
          <div className="flex flex-wrap gap-2 mb-3">
            {site.hero.badges.map((b) => <Pill key={b}>{b}</Pill>)}
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <a href="#contact" className="px-4 py-2 rounded-xl bg-sky-500 hover:bg-sky-400 font-bold btn-shine">5分で資料請求</a>
            <a href="#features" className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10">特徴を見る</a>
          </div>

          <div className="grid grid-cols-3 gap-3 mt-8">
            {site.hero.stats.map((s) => <Stat key={s.k} {...s} />)}
          </div>
        </div>
